...
function em_paginate($link, $total, $limit, $page=1, $data=array()){
	if($limit > 0){
	    ...
	    //Add the back and first buttons
		    ...
		    if($page == 2){
		    	$string .= ' <li><a class="prev page-numbers" href="'.$base_link.$base_querystring.'" title="2">&lt;</a> </li>';
		    }elseif($page > 2){
		...
		//Loop each page and create a link or just a bold number if its the current page
		    for ($i = $startPage ; $i < $startPage+$pagesToShow && $i <= $maxPages ; $i++){
	            if($i == $page || (empty($page) && $startPage == $i)) {
	                ...
	                $string .= '<li> <a class="page-numbers" href="'.$base_link.$base_querystring.'" title="'.$i.'">'.$i.'</a> </li>';
	            }else{
		...
		//Return the string
		    return apply_filters('em_paginate', '<span class="pagination" '.$data_atts.'>'.$string.'</span>');
	}
}
}
